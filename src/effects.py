...
@dataclass
class EffectContext:
    """Mutable per-segment effect-building context (W2-7).

    Replaces the former ~21-key ``ctx`` dict passed to every ``_fx_*`` builder
    and ``_transition_filters``. Field names are exactly the old dict keys and
    carry the same values/types; builders mutate ``filters`` (append),
    ``pack``, ``mirror_used`` and ``needs_scale_restore`` in place exactly as
    before. Construction passes every field by keyword (see
    ``build_effect_filters``), so field-definition order is irrelevant — no
    code iterates the context's fields.
    """

    clip: Dict            # the planned clip (or {}), read via clip.get(...)
    target: str
    energy: float
    k: float
    rng: random.Random
...
def _loudness_gain(ctx: EffectContext) -> float:
...
def _fx_punch_fill(ctx: EffectContext) -> None:
...
def _fx_punch_zoom(ctx: EffectContext) -> None:
...
def _fx_push_pull_zoom(ctx: EffectContext) -> None:
...
def _fx_dutch_tilt(ctx: EffectContext) -> None:
...
def _fx_shake(ctx: EffectContext) -> None:
...
def _fx_white_flash(ctx: EffectContext) -> None:
...
def _fx_sat_pulse(ctx: EffectContext) -> None:
...
def _fx_pixelize_burst(ctx: EffectContext) -> None:
...
def _fx_zoom_blur(ctx: EffectContext) -> None:
...
def _fx_motion_smear(ctx: EffectContext) -> None:
...
def _fx_strobe(ctx: EffectContext) -> None:
...
def _fx_trails(ctx: EffectContext) -> None:
...
def _fx_hue_sweep(ctx: EffectContext) -> None:
...
def _fx_posterize_flash(ctx: EffectContext) -> None:
...
def _fx_half_mirror(ctx: EffectContext) -> None:
...
def _fx_kaleido_quad(ctx: EffectContext) -> None:
...
def _fx_beat_flip(ctx: EffectContext) -> None:
...
def _fx_vignette_grain(ctx: EffectContext) -> None:
...
def _transition_filters(spec: Dict, side: str, duration: float, k: float,
                        target_size: Optional[Tuple[int, int]],
                        ctx: Optional[EffectContext] = None) -> List[str]:
