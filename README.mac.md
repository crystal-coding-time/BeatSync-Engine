...
Still images work too: `.jpg`, `.jpeg`, `.png`, `.webp`, `.bmp`. Each segment cut from a still gets its own gentle Ken Burns pan/zoom so photos read as footage. Sideways phone photos (EXIF rotation) are automatically shown upright, and transparent areas in PNG/WebP/GIF sources render as black instead of leaking stray colors. **HEIC is not supported** (Homebrew ffmpeg has no HEIF decoder) — export iPhone photos to JPEG first. In ProRes precise mode stills render as static frames (proxies stay pristine).
...
repeating per track. The final soundtrack is assembled sample-exactly at full quality. (Shuffle-mode's filename-derived seed uses the first song.)
...
Customize-effects panel), **The Cut** (cut density, source & visual variety, split-screen, crossfades,
...
- **Style** — Minimal (clean cuts, no effects), Music video (AMV), or Hype. See "Effect
  style" details below.
- **Output canvas** — the frame every render targets: 16:9 1080p (default), 16:9 4K, 9:16 Portrait, or Match best source (the highest-resolution upload decides the resolution *and* aspect, so one portrait phone clip can flip the whole video vertical). A fixed canvas makes the output predictable for YouTube/TV; pick Portrait deliberately for Shorts/Reels. Output FPS follows the highest-resolution *real video* source — stills and GIFs never set the render fps, so a low-fps GIF can't drag the whole render down even when it wins the resolution pick; set Custom FPS (Export tab) to override.
- **Frame fit** (Framing tab) — how sources with a different aspect ratio fill the canvas: Auto (default), Blurred background (undistorted over a blurred fill), or Letterbox. (Stretch is not offered in the UI; headless callers can pass `fit_mode='stretch'`.) Auto is subject-aware, in three tiers by how badly a source mismatches:
  ...
  Anamorphic sources are resampled to square pixels first, so they render undistorted. If the face model goes missing, re-fetch it with `.venv/bin/python scripts/fetch_yunet.py`; set `BEATSYNC_YUNET_MODEL=/nonexistent` to disable face detection entirely.
...
- **Content-aware effects** (Vibe tab) — **on by default** (untick to disable): effects check the footage before firing. Glitch/strobe/shake/zoom-blur/motion-smear skip clips with no measured motion or action (photos never get motion effects, no matter how action-packed they look), flash and posterize hits skip cuts the music doesn't actually hit, motion trails, hue sweeps and slow push-ins skip busy subjects, mirror and beat-flip effects skip face close-ups, saturation pulses leave a deliberately graded beauty shot alone, the rare fisheye and the film grain only appear on the energetic parts of the track, and every effect's chance of firing scales with how hard the music actually hits at that cut. Whip-pan transitions continue the outgoing clip's measured camera drift instead of picking a random direction, and never fire across a photo boundary. Uses the vision tags when stage 5 ran and measured optical-flow motion otherwise; clips with no data are unaffected.
- **Smart photo motion** (Vibe tab) — **on by default** (untick to disable): photos move with the music instead of a generic Ken Burns drift. Calm parts get a slow pan that travels toward the detected face/subject, builds get a push-in centered on it, drops get a faster, decisive punch-in. Deterministic like everything else; off = the generic drift.
...
- **Source variety** (The Cut tab) — how hard the planner spreads cuts across your uploads. 0 = pure quality auction (the best-scoring moments win, some files may never appear), 1 = near-even spread. Default 0.4 guarantees every usable source appears at least once while keeping the best footage on the big drops; above 0 the spread uses proportional-fair pressure, so a genuinely stronger source keeps a bounded lead instead of being ground down to round-robin. The render log prints a per-source usage histogram. When there simply isn't enough footage to avoid reusing a moment, reuse does not replay the same frames: the planner remembers which slice of each clip it has already spent and slides the next visit onto an unused part of that shot, so a short library stops feeling like it loops. That memory applies in every mode, including ProRes, and at any variety setting.
- **Visual variety** (The Cut tab) — avoids runs of *visually similar* shots even across different files (e.g. five wide landscapes in a row). A small local DINOv2 model embeds one frame per candidate moment and clusters the footage (CPU-only, deterministic, cached per source); the planner then penalizes picking from a just-used cluster or anything too cosine-similar to the last few picks. Fetch the model once with `.venv/bin/python scripts/fetch_dinov2.py` (~87 MB into `models/`); without it the slider silently does nothing. 0 = off. Default 0.4. `BEATSYNC_DISABLE_EMBED=1` is the kill switch.
- **Cut density** (The Cut tab) — how hard the *cutting rate* follows the music. This is the only setting that changes **when** the video cuts, so moving it re-plans the whole edit. At 0 the edit cuts at a near-constant rate regardless of what the song is doing; the default 0.55 is noticeably busier on loud material. Each section is judged against the track's own peak rather than an absolute loudness, so a quiet mix gets the same shape as a loud one: drops and choruses approach one cut per beat while intros and breakdowns keep their slow pace. Near the top of the range the hottest sections also cut on half beats. Live in every mode, including ProRes. Changing it re-runs the beat analysis rather than reusing the cached cut list.
- **Media-aware selection** (The Cut tab) — **on by default** (untick to disable); made for mixed uploads (videos + GIFs + photos in one folder). Low-resolution clips *and photos* pay a growing penalty the more they'd have to be blown up to fill the canvas (so one 360p GIF stops landing on your biggest moments), GIFs whose loop visibly "pops" pay more the more times a segment would make them wrap (a barely-over loop costs almost nothing, many wraps cost double) while seamless loops stay fair game anywhere, and low-frame-rate sources get their motion measured across every frame of the shot instead of a handful of samples. Applies to which source serves each segment in every mode, including ProRes.
...
- **Speed ramps (experimental, The Cut tab)** — opt-in: slow-motion drifts through calm segments, speed-ups through builds, decelerating hits into drops, and rare freeze-frames. Occasionally a drop cut gets an anticipation ramp: the outgoing clip's last half-second rushes ahead, the downbeat lands in slow motion (0.4–0.5×) and eases back to full speed — rare by design (roughly one per eight segments at most, and only where both sides have the footage to support it). Deep slow-mo (below 0.6×) on ordinary 24–30fps footage is motion-interpolated (ffmpeg `minterpolate`) so it stays smooth instead of juddering — it costs a few extra seconds of render per affected segment, so the planner keeps it rare. Frame-count-safe by construction (every retimed segment is verified after extraction); skips images, GIFs, sub-24fps sources, and ProRes mode.
