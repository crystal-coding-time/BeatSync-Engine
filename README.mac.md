...
- **Media-aware selection** (The Cut tab) — opt-in, off by default; made for mixed uploads (videos + GIFs + photos in one folder). Low-resolution clips *and photos* pay a growing penalty the more they'd have to be blown up to fill the canvas (so one 360p GIF stops landing on your biggest moments), GIFs whose loop visibly "pops" pay more the more times a segment would make them wrap (a barely-over loop costs almost nothing, many wraps cost double) while seamless loops stay fair game anywhere, and GIF motion is measured at its native frame rate instead of reading as garbage. Applies to which source serves each segment in every mode, including ProRes.
...
After updating, the first render re-analyzes each source once (the analysis cache gained camera-direction fields); later renders reuse the cache as before.
