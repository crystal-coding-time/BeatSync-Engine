...
- **Content-aware effects** (Vibe tab) — opt-in, off by default: effects check the footage before firing. Glitch/strobe/shake skip clips with no measured motion or action, motion trails and hue sweeps skip busy subjects, mirror effects skip face close-ups, and every effect's chance of firing scales with how hard the music actually hits at that cut. Uses the vision tags when stage 5 ran and measured optical-flow motion otherwise; clips with no data behave exactly as before.
...
- **Media-aware selection** (The Cut tab) — opt-in, off by default; made for mixed uploads (videos + GIFs + photos in one folder). Low-resolution clips pay a growing penalty the more they'd have to be blown up to fill the canvas (so one 360p GIF stops landing on your biggest moments), GIFs whose loop visibly "pops" prefer segments they can cover in a single pass while seamless loops stay fair game anywhere, and GIF motion is measured at its native frame rate instead of reading as garbage. Photos are exempt from the resolution penalty. Applies to which source serves each segment in every mode, including ProRes.
