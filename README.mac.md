...
- **Cut density** (The Cut tab) — how hard the *cutting rate* follows the music. This is the only setting that changes **when** the video cuts, so moving it re-plans the whole edit and nothing about a previous render is comparable. 0 reproduces the previous pacing exactly (useful as an A/B reference), and the default 0.55 is noticeably busier on loud material. Each section is judged against the track's own peak rather than an absolute loudness, so a quiet mix gets the same shape as a loud one: drops and choruses approach one cut per beat while intros and breakdowns keep their slow pace — on the smoke track the gap between calmest and busiest section widens from 1.3× to 2.4× (4.3× at 1.0). Near the top of the range the hottest sections also unlock half-beat cutting. Live in every mode, including ProRes. Changing it re-runs the beat analysis rather than reusing the cached cut list.
- **Media-aware selection** (The Cut tab) — **on by default** (untick for the old behavior); made for mixed uploads (videos + GIFs + photos in one folder). Low-resolution clips *and photos* pay a growing penalty the more they'd have to be blown up to fill the canvas (so one 360p GIF stops landing on your biggest moments), GIFs whose loop visibly "pops" pay more the more times a segment would make them wrap (a barely-over loop costs almost nothing, many wraps cost double) while seamless loops stay fair game anywhere, and low-frame-rate sources get their motion measured across every frame of the shot instead of a handful of samples. Applies to which source serves each segment in every mode, including ProRes.
...
After updating, the first render re-analyzes each source once (clip-motion measurement was rebuilt — it now compares neighbouring frames of the source rather than frames a half-second apart, which is the only spacing at which it actually tracks how fast a shot moves; before this, genuinely fast footage could measure *calmer* than a gentle pan and drift onto the quiet parts of the song). Expect the matching of shots to music energy to change on your first render after the update; later renders reuse the cache as before.

Also new: **Content-aware effects**, **Smart photo motion** and **Media-aware selection** now start ticked. They used to ship off, which meant the checks that stop glitch effects landing on a still photo (or a 360p GIF landing on your biggest drop) never ran unless you found the boxes. Renders made with the stock settings will therefore differ from before the update; untick any of the three to get the previous behavior back exactly.
